...
def SLcheckFilterSizes(
    ...
    configs = [
        # Configuration 1
        {"dfilt": "dmaxflat4", "g": weights},
        # Configuration 2
        {"dfilt": "cd", "g": weights},
        # Configuration 3
        {"dfilt": "cd", "g": MakeONFilter("Coiflet", 1)},
        # Configuration 4
        {"dfilt": "cd", "g": MakeONFilter("Daubechies", 4)},
        # Configuration 5
        {"dfilt": "oqf_362", "g": MakeONFilter("Daubechies", 4)},
        # Configuration 6
        {"dfilt": "oqf_362", "g": MakeONFilter("Haar", 4)},
    ]
    ...
    for cfg in configs:
        h0, _ = dfilters(cfg["dfilt"], "d")
        directionalFilter = modulate2(h0, "c")
        filter_setups.append(
            {
                "directionalFilter": directionalFilter,
                "scalingFilter": cfg["g"],
                "waveletFilter": MirrorFilt(cfg["g"]),
                "scalingFilter2": cfg["g"],
            }
        )

    success = False
    for k, setup in enumerate(filter_setups):
        # check 1
        lwfilter = setup["waveletFilter"].size
        lsfilter = setup["scalingFilter"].size
        lcheck1 = lwfilter
        for _ in range(shearLevels.size - 1):
        ...
        # check 2
        rows_dfilt, cols_dfilt = setup["directionalFilter"].shape[:2]
        lcheck2 = (rows_dfilt - 1) * np.power(2, max(shearLevels) + 1) + 1

        lsfilter2 = setup["scalingFilter2"].size
        lcheck2help = lsfilter2
        for _ in range(max(shearLevels)):
            lcheck2help = lsfilter2 + 2 * lcheck2help - 2
        lcheck2 = lcheck2help + lcheck2 - 1
        if lcheck2 > cols or lcheck2 > rows or cols_dfilt > cols or cols_dfilt > rows:
            continue
        success = True
        break
    ...
    if not success:
        raise ValueError(
            f"The specified Shearlet system is not available for data of size {rows}x{cols}. Try decreasing the number of scales and shearings."
        )
    if k > 0:
        print(
            f"Warning: The specified Shearlet system was not available for data of size {rows}x{cols}. Filters were automatically set to configuration {k} (see SLcheckFilterSizes)."
        )
...
def SLdshear(inputArray, k, axis):
    ...
    if axis == 0:
        for col in range(cols):
            shearedArray[:, col] = np.roll(
                inputArray[:, col], int(k * np.floor(cols / 2 - col))
            )
    else:
        for row in range(rows):
            shearedArray[row, :] = np.roll(
...
def SLgetShearletIdxs2D(shearLevels, full=0, *args):
    ...
    shearings = np.asarray(
        range(-np.power(2, np.max(shearLevels)), np.power(2, np.max(shearLevels)) + 1)
    )
    cones = np.array([1, 2])
    ...
    for cone in np.intersect1d(np.array([1, 2]), cones):
        for scale in np.intersect1d(np.asarray(range(1, len(shearLevels) + 1)), scales):
            for shearing in np.intersect1d(
                np.asarray(
                    range(
                        -np.power(2, shearLevels[scale - 1]),
                        np.power(2, shearLevels[scale - 1]) + 1,
                    )
                ),
                shearings,
            ):
                if (
                    (full == 1)
                    or (cone == 1)
                    or (np.abs(shearing) < np.power(2, shearLevels[scale - 1]))
                ):
    ...
    for j in range(nShearlets):
        ...
        if cone == 0:
            ...
            shearlets[:, :, j] = preparedFilters["cone1"]["wedge"][
                ...
                preparedFilters["cone1"]["bandpass"][:, :, scale - 1]
            )
        else:
...
def SLgetWedgeBandpassAndLowpassFilters2D(
    ...
    # normalize filters
    directionalFilter = directionalFilter / np.sum(np.absolute(directionalFilter))
    ...
    filterHigh[-1] = waveletFilter.copy()
    filterLow[-1] = scalingFilter.copy()  # this filter corresponds to h_1 on page 11
    ...
    # w_j on page 14
    filterLow2[-1] = scalingFilter2.copy()
    ...
    for shearLevel in np.unique(shearLevels):
        ...
        lowpassHelp = SLpadArray(
            filterLow2[-1 - max(shearLevel - 1, 0)],
        ...
        # frequency cone
        for k in range(-(2**shearLevel), 2**shearLevel + 1):
            ...
            wedge[shearLevel][:, :, 2**shearLevel - k] = fftlib.fftshift(
                fftlib.fft2(
                    fftlib.ifftshift(
                        2**shearLevel
                        * wedgeUpsampledSheared[
                            :,
                            : 2**shearLevel * cols : 2**shearLevel,
                        ]
                    )
                )
            )
...
def SLpadArray(array, size):
    if np.isscalar(size):
        size = np.array([size])

    while array.ndim < len(size):
        array = array[None, :]

    diff = size - np.array(array.shape)
    if np.any(diff < 0):
        raise ValueError(
            f"input array (shape:${array.shape}) is bigger than target size (shape:${size}) in some dimension"
        )

    pad_width = tuple((int(np.ceil(half)), int(np.floor(half))) for half in diff / 2)
...
def SLupsample(array, dims, nZeros):
    ...
    if array.ndim == 1:
        ...
        if dims == 0:
            sys.exit(
                "SLupsample behaves like in Matlab, so chose dims = 1 or dims = 2."
            )
        if dims == 1:
