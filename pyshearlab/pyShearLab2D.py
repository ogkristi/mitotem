...
def SLgetShearletSystem2D(
    ...
    preparedFilters = SLprepareFilters2D(
        rows, cols, nScales, shearLevels, directionalFilter, quadratureMirrorFilter
    )
...
def SLsheardec2D(X, shearletSystem):
    ...
    for j in range(shearletSystem["nShearlets"]):
        coeffs[:, :, j] = fftlib.fftshift(
            fftlib.ifft2(
                fftlib.ifftshift(Xfreq * np.conj(shearletSystem["shearlets"][:, :, j]))
            )
        )
    ...
    if np.max(np.abs(np.imag(coeffs))) > err_bound:
        ...
        print(
            "Data is probably not real-valued. Largest magnitude: "
            + str(np.max(np.abs(np.imag(coeffs))))
        )
...
def SLshearrec2D(coeffs, shearletSystem):
    ...
    for j in range(shearletSystem["nShearlets"]):
        X = (
            X
            + fftlib.fftshift(fftlib.fft2(fftlib.ifftshift(coeffs[:, :, j])))
            * shearletSystem["shearlets"][:, :, j]
        )

    X = fftlib.fftshift(
        fftlib.ifft2(
            fftlib.ifftshift((np.divide(X, shearletSystem["dualFrameWeights"])))
        )
    )
    ...
    if np.max(np.abs(np.imag(X))) > err_bound:
        warnings.warn(
            f"""
        Magnitude in imaginary part exceeded {err_bound}.
        Data is probably not real-valued. Largest magnitude: {np.max(np.abs(np.imag(X)))}
...
def SLshearadjoint2D(coeffs, shearletSystem):
    ...
    for j in range(shearletSystem["nShearlets"]):
        X += np.conj(shearletSystem["shearlets"][:, :, j]) * fftlib.fftshift(
            fftlib.fft2(fftlib.ifftshift(coeffs[..., j]))
        )
...
def SLshearrecadjoint2D(X, shearletSystem):
    ...
    # STUFF
    Xfreq = np.divide(
        fftlib.fftshift(fftlib.fft2(fftlib.ifftshift(X))),
        shearletSystem["dualFrameWeights"],
    )
    coeffs = np.zeros(shearletSystem["shearlets"].shape, dtype=complex)
