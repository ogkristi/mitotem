...
def detect_scale(img: np.ndarray, reader):
    ...
    scalebar = open_rc(bin, kernel=cv.getStructuringElement(cv.MORPH_RECT, (kwidth, 1)))
...
def get_cristae_mask(
    mito: np.ndarray, mask: np.ndarray, operations: list[Callable[..., np.ndarray]]
):
    img = np.copy(mito)
...
def seq_line_open_rc(src, size):
    ...
    for angle in np.arange(-45, 45.1, 22.5):
        rm = cv.getRotationMatrix2D(center, angle, 1.0)
...
def contour_smoothing(src: np.ndarray, p: float):
    ...
    for ctr in contours:
        ...
        fd = np.fft.fftshift(
            cv.dft(np.float32(ctr), flags=cv.DFT_COMPLEX_INPUT + cv.DFT_COMPLEX_OUTPUT)
        )
        if l != 0:
            fd[:l] = [[0.0, 0.0]]
            fd[-l:] = [[0.0, 0.0]]
        newctr = np.int32(
            cv.idft(np.fft.ifftshift(fd), flags=cv.DFT_COMPLEX_OUTPUT + cv.DFT_SCALE)
        )
...
def demaze(img: np.ndarray) -> np.ndarray:
    ...
    cx, cy = (
        n // 2,
        m // 2,
    )  # centers of four notch filters (0,-cy),(-cx,0),(cx,0),(0,cy)
    H = (
